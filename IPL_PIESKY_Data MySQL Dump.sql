...
SELECT 
    bidder_id,
    (COUNT(CASE
        WHEN bid_status = 'Won' THEN bid_status
    END) / COUNT(bidder_id)) * 100 Win_Percent
FROM
    ipl_bidding_details
GROUP BY bidder_id
...
SELECT 
    ...
    s.stadium_name,
    s.city,
    COUNT(m.stadium_id) No_of_matches
FROM
    ipl_stadium s
        JOIN
...
SELECT 
    stadium_id,
    COUNT(CASE
        WHEN toss_winner = match_winner THEN match_winner
    END) / COUNT(toss_winner) toss_match_winner
FROM
    ipl_match m
        JOIN
    ipl_match_schedule s ON m.match_id = s.match_id
GROUP BY stadium_id
ORDER BY stadium_id;
...
SELECT 
    bid_team, team_name, COUNT(team_name) num_of_bids
FROM
...
SELECT 
    match_id, match_winner winning_team_id, team_name
FROM
    ipl_match m
        JOIN
...
SELECT 
    player_id, player_role, t.team_name
FROM
    ipl_team_players p
        RIGHT JOIN
    ipl_team t ON p.team_id = t.team_id
WHERE
    p.team_id = 5
        AND p.player_role = 'Bowler';

-- 8. How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.

SELECT 
    team_name, COUNT(player_role) num_of_all_rounders
FROM
    ipl_team_players p
        RIGHT JOIN
    ipl_team t ON p.team_id = t.team_id
